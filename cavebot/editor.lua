...
-- Altere as funções originais no final do seu arquivo para estas:
CaveBot.Editor.show = function()
...
-- Impedimos que a função nativa esconda o painel permanentemente
CaveBot.Editor.hide = function()
  if CaveBot.Editor.ui then
    CaveBot.Editor.ui:show() -- Força a continuar mostrando em vez de dar hide()
  end
end

-- Nova Macro para injetar e forçar que o painel esteja sempre ativo
macro(100, function()
  if CaveBot.Editor.ui then
    -- Se o painel principal do editor estiver escondido por qualquer motivo, força exibição
    if not CaveBot.Editor.ui:isVisible() then
      CaveBot.Editor.ui:show()
    end
    
    -- Mantém o botão de controle visual (Create CaveBot) sempre marcado como ativo (Verde)
    local botWindow = modules.game_bot.botWindow
    if botWindow then
      local titleButton = botWindow:recursiveGetChildById('createCavebotBtn') 
                        or botWindow:recursiveGetChildById('createCavebot')
                        or botWindow:recursiveGetChildById('editorButton')
      if titleButton and not titleButton:isOn() then
